    ...
    public static class LoopingCollectionToWriteFile
    {
        ...
        */
        public static void LoopingADictionaryToWriteAFile()
        {
            ...
            string directory = Environment.CurrentDirectory; //this is the current directory where program is running, not where the files live
            string filename = "programminglanguages.txt"; //name of the new fiel
            string path = Path.Combine(directory, filename); //make path

            // Open up a streamwriter for the file to write to
            using (StreamWriter sw = new StreamWriter(path)) //going to write
            {
                // Loop through each key-value pair and print it out to the file
                foreach (KeyValuePair<string, double> kvp in programmingLanguages) //loop through each val and print
                {
