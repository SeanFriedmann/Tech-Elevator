...
namespace TechElevator.Exercises.LogicalBranching
{
    ...
    public class ShippingTotal
    {
        ...
        public double CalculateShippingTotal(int weightInPounds, bool hasDiscount)
        {
            ...
            {
                return ((weightInPounds * UpTo40PoundRate) - ((weightInPounds * UpTo40PoundRate) *.10));
            }
            if (weightInPounds > 40 && hasDiscount == false)
            {
                return (MaxWeightPounds * UpTo40PoundRate) + ((weightInPounds - MaxWeightPounds) * Over40PoundRate);
            ...
            if (weightInPounds > 40 && hasDiscount == true)
            {
                return ((MaxWeightPounds * UpTo40PoundRate) + (weightInPounds - MaxWeightPounds) * Over40PoundRate) - (((MaxWeightPounds * UpTo40PoundRate) + (weightInPounds - MaxWeightPounds) * Over40PoundRate) * .10);
            }
            return 0;
        }
        ...
        public double CalculateShippingTotal(int weightInPounds, double discountPercentage)
        {
            ...
            //{
            //    return ((weightInPounds / 2) - ((weightInPounds / 2) * discountPercentage));
            //}
            //if (discountPercentage == 0)
            //{
            ...
            if (weightInPounds > 40 && discountPercentage > 0)
            {
                return ((MaxWeightPounds * UpTo40PoundRate) + (weightInPounds - MaxWeightPounds) * Over40PoundRate) - (((MaxWeightPounds * UpTo40PoundRate) + (weightInPounds - MaxWeightPounds) * Over40PoundRate) * discountPercentage);
            }
