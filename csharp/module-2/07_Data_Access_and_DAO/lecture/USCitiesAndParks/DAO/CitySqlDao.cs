...
namespace USCitiesAndParks.DAO
{
    public class CitySqlDao : ICityDao
    {
        ...
        //private property for connection

        public CitySqlDao(string connString)
        {
            connectionString = connString;
            //parameter for connectin string
            //CitySQLDao instantiated in program.cs, thats where the connection string comes from
        }
        //method to retrrieve city data from the database
        public City GetCity(int cityId)
        {
            City city = null;

            using (SqlConnection conn = new SqlConnection(connectionString))
                //using: auto close when finished
                //conn: name, just a variable name for the sql connection
                //new SQLConnection: new connection
                //connectionString: using this connection string
            {
                conn.Open();
                //opens connection to the database using our SQLConnection var name
                ...
                //SQLCommand: build a sql query
                //cmd: name of the query
                //("SELECT..."): SQL command 
                // ..., conn): SQL connection from up top at the using statement
                // WHERE city_id = @city.id: where city is anything outof the column
                //@city_id: placeholder/ SQL parameter

                cmd.Parameters.AddWithValue("@city_id", cityId);
                //cmd: SQL command from earlier
                //.Parameters: parameters
                //AddWithValue: add a param with a value
                //@city_id: placeholder
                //cityId: value, passed in from GetCity method


                SqlDataReader reader = cmd.ExecuteReader();
                //create SQL data reader using the SQLCommand variable name from earlier, runs the query against the DB

                if (reader.Read()) 
                    //if we can read the result set (no errors, data rows exist)
                {
                    city = CreateCityFromReader(reader);
                    //city object created from the results, return city object
                    //reference a method created at the bottom of the cs file
                }
            }
            return city; 
            //return city object
        ...
        public IList<City> GetCitiesByState(string stateAbbreviation)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    ...
                    cities.Add(city);
                    //while there is something to read add a city to the list
                }
            }
