...
namespace AuctionApp.Services
{
    public class AuctionApiService
    {
        ...
        {
            RestRequest request = new RestRequest("auctions");
            request.AddJsonBody(newAuction); //change request to JSON data
            IRestResponse<Auction> response = client.Post<Auction>(request); //create response
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                throw new HttpRequestException("Error occurred - unable to reach server.", response.ErrorException);
            }
        ...
        public Auction UpdateAuction(Auction auctionToUpdate)
        {
            ...
            request.AddJsonBody(auctionToUpdate); //change request to JSON data for data being updated
            IRestResponse<Auction> response = client.Put<Auction>(request); //will override whole object on the server
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                throw new HttpRequestException("Error occurred - unable to reach server.", response.ErrorException);
            }
