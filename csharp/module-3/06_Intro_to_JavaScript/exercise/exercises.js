...
*/
		function frontAgain(string){
			...
			//let firstTwo = word.substring(0, 1);
			//let lastTwo = word.substr(word.length - 2)
			//
			if (lastTwo == firstTwo){
				return true;
			}
...
*/
		...
		function cigarParty(x,bool){
			...
			else if (x >= 40 && x <= 60 && bool == false){
				return true;
			}
			else if (x >= 40 && bool == true)
			{
				return true;
			}
		}
...
*/
		function filterMultiplesOfX(numbers, divider){
			let multiples = [];
