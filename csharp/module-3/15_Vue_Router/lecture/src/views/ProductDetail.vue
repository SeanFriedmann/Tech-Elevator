...
export default {
    ...
    computed: {
        product() {
            ...
            //use the getter for product in the store
            //so that it always gives me the product
            //with the active product id
        }
    },

    //want to set active product in the store to the id in my route
    created(){
        //get the product id from the route
        const activeProductId = this.$route.params.id;
        //commit a mutation to change value in the store
        this.$store.commit('SET_ACTIVE_PRODUCT', activeProductId);
    }
    //created is a lifecycle hook that run when the view is 
    //being made so that this code runs beofre the view
