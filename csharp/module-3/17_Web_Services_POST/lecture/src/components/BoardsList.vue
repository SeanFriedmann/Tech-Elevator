...
export default {
  ...
  methods: {
    ...
    saveNewBoard() { //dont need response object
      this.isLoading = true;
      ...
      boardsService.addBoard(this.newBoard).then(() => {
        ...
        if(error.response) {
          //if error has response we know we made it to server
          this.errorMsg = 'Error adding a new board, response received from server was ' + 
          error.response.statusText + '.';
        }
        else if (error.request){
          //we never got a response, problem reaching server
